...
from .models import ALL_SENSORS, string_sensor_descriptions
...
async def async_setup_entry(
    ...
    # Per-string (MPPT) sensors, one set per discovered string.
    for index in range(1, coordinator.num_strings + 1):
        for description in string_sensor_descriptions(index):
            if description.key in coordinator.data:
                entities.append(
                    FroniusModbusSensor(
                        coordinator,
                        description,
                        unique_id_base,
                        translation_placeholders={"string": str(index)},
                    )
                )
...
class FroniusModbusSensor(FroniusModbusEntity, SensorEntity):
    ...
    def native_value(self) -> int | float | str | None:
        ...
        if self.entity_description.device_class == SensorDeviceClass.ENUM:
            # Per-string state is already decoded to text; the inverter
            # operating state is a raw enum int that needs mapping.
